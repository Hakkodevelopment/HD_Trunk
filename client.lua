Citizen.CreateThread(function()
    while true do
        ...
        local veh = VehicleInFront(plyPos)
        local trunk = GetEntityBoneIndexByName(veh, 'boot')
        if trunk ~= -1 then
            local coords = GetWorldPositionOfEntityBone(veh, trunk)
            if #(plyPos - coords) <= 1.5 then
                if not inTrunk then
                    if GetVehicleDoorAngleRatio(veh, 5) < 0.9 then
                        DrawText3D(coords.x, coords.y, coords.z, '[~g~H~w~] Open')
